...
class AutoregressiveTree:
    ...
    def WN_func(self, uN_, SN, W0, N):
        ...
        WN = (
            W0 + SN + ((self._alpha_u * N) / (self._alpha_u + N)) * np.dot(temp, temp.T)
        )
        return WN
    ...
    def WN_d_func(self, u0_, uN_d_, SN_d_, W0_, N_):
        ...
        WN_ = (
            W0_
            + SN_d_
            + ((self._alpha_u * N_) / (self._alpha_u + N_)) * np.dot(temp, temp.T)
        )
        return WN_
    ...
    def pds_func(self, N, W0, WN):
        pds = (
            ...
            + (
                self.c_func(self._p + 1, self._alpha_W + N)
                / self.c_func(self._p + 1, self._alpha_W)
            )
            * (det(W0) ** (self._alpha_W / 2))
            * (det(WN) ** (-(self._alpha_W + N) / 2))
        )
        return pds
    ...
    def pd_s_func(self, u0_, N_, W0_, WN_):
        pds = (
            ...
            + (
                self.c_func(self._p + 1, self._alpha_W - 1 + N_)
                / self.c_func(self._p + 1, self._alpha_W - 1)
            )
            * (det(W0_) ** ((self._alpha_W - 1) / 2))
            * (det(WN_) ** (-(self._alpha_W - 1 + N_) / 2))
        )
        return pds
